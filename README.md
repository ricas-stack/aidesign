这是一个模仿小红书（Xiaohongshu）网页版界面的单页 HTML 代码。

**主要特点：**
1.  **瀑布流布局 (Masonry Layout)**：使用 CSS `column-count` 实现经典的小红书笔记卡片交错排列效果。
2.  **响应式设计**：随着屏幕宽度变化，自动调整列数（5列 -> 2列）。
3.  **视觉风格**：复刻了顶部导航栏、品牌红色 (`#ff2442`)、圆角搜索框和卡片样式。
4.  **动态模拟**：使用 JavaScript 自动生成随机高度的图片（利用 Lorem Picsum 服务）和模拟数据，并包含简单的无限滚动加载功能。

你可以直接将以下代码保存为 `.html` 文件，然后在浏览器中打开即可预览。
